    ...
    pub fn decode(mut bytes: Bytes) -> Result<Self, PacketError> {
        ...
        if level != 4 {
            return Err(PacketError::new(
                PacketErrorKind::ProtocolLevel,
                format!("{} is not a valid protocol level", level),
            ));
        }
        ...
        (client_id, bytes) = decode_utf8(bytes)?;
        ...
        if conn_flags.will() {
            // ewwww....
            let topic: String;
            (topic, bytes) = decode_utf8(bytes)?;

            let message: String;
            (message, bytes) = decode_utf8(bytes)?;
        ...
        if conn_flags.user_name() {
            let string: String;
            (string, bytes) = decode_utf8(bytes)?;
...
impl ConnectFlags {
    ...
    pub fn from_byte(byte: u8) -> Result<Self, PacketError> {
        if (byte & WILL_QOS_BITS >> 3) > 3 {
            return Err(PacketError::new(
                PacketErrorKind::WillQoS,
                String::from("Invalid 'will_QoS"),
            ));
        }
        if byte & RESERVED_BIT == RESERVED_BIT {
            return Err(PacketError::new(
                ...
                format!(
                    "Connect flag bits has reserved (0b0000_0001) bit set: {}",
                    byte
                ),
            ));
        }
        if byte & WILL == 0 {
            if byte & 0b0011_1000 != 0 {
                return Err(PacketError::new(
                PacketErrorKind::WillQoS,
                format!("Connect flag bits does not have 'will' bit set (0b0000_0100), but 'will_retain' or 'will_qos' bits (0b0011_1000) are set: {}", byte)));
            }
        }
        ...
        if byte & PASSWORD == PASSWORD {
            if byte & !USERNAME == USERNAME {
                return Err(PacketError::new(
                    PacketErrorKind::UsernamePassword,
                    format!("Password bit is set and Username bit is unset"),
                ));
            }
        }
...
impl Protocol {
    pub fn from_bytes(bytes: Bytes) -> Result<(Self, Bytes), PacketError> {
        ...
        match &protocol_name.as_str() {
            ...
            _ => {
                return Err(PacketError::new(
                    PacketErrorKind::ProtocolName,
                    format!("{}, is not a valid protocol name", protocol_name),
                ))
            }
        }
    }
