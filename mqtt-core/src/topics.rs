    ...
    pub fn from_str(str: &'_ str) -> Result<Self, PacketError> {
        if str.len() == 0 {
            return Err(PacketError::new(
                ...
                format!("TopicFilter str must be at least once character long."),
            ));
        }
...
impl TopicName {
    ...
    pub fn from_str(str: &'_ str) -> Result<Self, PacketError> {
        if str.len() == 0 {
            return Err(PacketError::new(
                PacketErrorKind::MalformedTopicName,
                format!("TopicName str must be at least once character long."),
            ));
        }
...
mod test {
    ...
    #[test]
    fn topics_begining_with_dollar_sign() {
